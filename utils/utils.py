...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    for dirpath, _, files in os.walk(folder):
        for filename in files:
            im = load_img(os.path.join(dirpath,filename))
            img = img_to_array(im)
            img = smart_resize(img, input_size)
            img = img.reshape([1,224,224,3])
            pred = model.predict(img)
            predicted_class_arg = pred.argmax()
            predicted_class = class_names[predicted_class_arg]
